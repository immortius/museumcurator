...
public class MuseumCuratorMod {
    ...
    public void onServerLogin(PlayerEvent.PlayerLoggedInEvent event) {
        MESSAGE_CHANNEL.send(PacketDistributor.PLAYER.with(() -> (ServerPlayer)(event.getEntity())), new LogOnMessage(MuseumCollections.getCollections(), ChecklistState.get(event.getEntity().getServer()).getCheckedItems()));
    }
